...
class Category
{
    const QRY_DATAGRID_BROWSE =
             'SELECT i.id, c.name,  i.sequence
              ...
              INNER JOIN vacancies_category_content as c  on i.id = c.category_id
        ...
        public static function getURL($URL, $language, $id = null)
        {
            ...
            if ($id === null) {
                ...
                if ((bool) $db->getVar(
                    'SELECT 1
                     FROM vacancies AS i
                     INNER JOIN vacancies_category_content AS m ON i.id = m.category_id
        ...
        */
        ...
        {
            BackendModel::get('database')->delete('vacancies_categories', 'id = ?', (int) $id);
            BackendModel::get('database')->delete('vacancies_category_content', 'category_id = ?', (int) $id);
            BackendModel::get('database')->delete('vacancies_linked_catgories', 'category_id = ?', (int) $id);
        }
        ...
        */
        public static function get($id)
        {
           ...
           $return['content'] = (array) $db->getRecords(
               'SELECT i.* FROM vacancies_category_content AS i
               WHERE i.category_id = ?',
    ...
    public static function getForDropdown()
    {
        $allCategories = (array) BackendModel::get('database')->getRecords('SELECT i.id, c.name, i.parent_id
              FROM vacancies_categories AS i
              INNER JOIN vacancies_category_content AS c on c.category_id = i.id
               WHERE c.language = ?
    ...
    public static function buildTreeForMultiCheckbox(array &$tree, array $categories, $parentId = 0, $level = 0)
    {
        ...
        */
        public static function insert(array $item)
        {
            ...
            return (int) BackendModel::get('database')->insert('vacancies_categories', $item);
        }

    public static function insertContent(array $content)
    {
        foreach ($content as &$item) {
            if (BackendModel::get('fork.settings')->get('Vacancies', 'make_widget_per_category', true) == true) {
                $data = [
                        'id' => $item['category_id'],
            ...
            BackendModel::get('database')->insert('vacancies_category_content', $item);
        }
    }
        ...
        */
        public static function update(array $item)
        {
            ...
            BackendModel::get('database')->update(
               'vacancies_categories', $item, 'id = ?', (int) $item['id']
           );
        }
    ...
    public static function updateContent(array $content, $id)
    {
        ...
        foreach ($content as $language => $row) {
            if (BackendModel::get('fork.settings')->get('Vacancies', 'make_widget_per_category', true) == true && $row['extra_id']) {
                $data = [
                       'id' => $row['category_id'],
            ...
            $db->update('vacancies_category_content', $row, 'category_id = ? AND language = ?', array($id, $language));
        }
    }
