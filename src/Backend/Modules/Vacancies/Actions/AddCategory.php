...
class AddCategory extends ActionAdd
{
    ...
    protected function loadForm()
    {
        ...
        $categories = BackendVacanciesCategoryModel::getForDropdown();
    ...
    protected function validateForm()
    {
        if ($this->frm->isSubmitted()) {
            ...
            if ($this->frm->isCorrect()) {
                ...
                $item['sequence'] = BackendVacanciesCategoryModel::getMaximumSequence() + 1;
                $item['parent_id']  = $fields['parent_id']->getValue();
                $item['id'] = BackendVacanciesCategoryModel::insert($item);

                if ($item['parent_id'] == 0) {
                    $item['path'] = '/' . $item['id'] . '/';
                    BackendVacanciesCategoryModel::update($item);
                } else {
                    $category = BackendVacanciesCategoryModel::get($item['parent_id']);
                ...
                foreach ($this->languages as $language) {
                    ...
                    $specific['url'] =  BackendVacanciesCategoryModel::getURL(CommonUri::getUrl($specific['name']), $language['abbreviation']);
                ...
                // insert it
               BackendVacanciesCategoryModel::insertContent($content);
